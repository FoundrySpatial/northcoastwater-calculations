...
WITH session_gage_id AS(
    ...
    SELECT *
    FROM cwat_app.fill_missing_years(%(input_data)s)
    ), gage_data_julian AS (
    SELECT
        datestamp,
        val,
        ...
        CASE
            WHEN EXTRACT(MONTH FROM datestamp) >= 10 THEN
                (EXTRACT(YEAR FROM datestamp))::int + 1
            ELSE
...
VALUES (%(user_id_in)s, %(session_id_in)s, '{{}}'::json, '{{}}'::json, '{{}}'::json, (SELECT jsonf from formatted_json))
ON CONFLICT (user_id, cda_session_id)
DO UPDATE
SET unimpaired_gage_data = (SELECT jsonf from formatted_json)
WHERE gage_senior_diverter_csv.cda_session_id = %(session_id_in)s and gage_senior_diverter_csv.user_id = %(user_id_in)s;
"""