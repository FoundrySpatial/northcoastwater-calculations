generate_watershed_for_pod_and_store_query = """
    WITH INPUT(geojson) AS (
        values (%s)
    ), point_to_analyse AS (
    SELECT
        ST_GeomFromGeoJSON(geojson) as pointgeom4326
    FROM
        input
    ), point_catchment AS (
        SELECT
    ...
    INSERT INTO cwat_app.pod_watersheds
    SELECT
        %s::integer as session_id,
        watershed_geom4326
    FROM
        watershed
